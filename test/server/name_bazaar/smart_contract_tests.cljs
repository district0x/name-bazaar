...
(deftest contracts-setup
  (async done
    (go
      (is (= (contract-address :ens) (second (<! (registrar/ens)))))
      (is (= (contract-address :ens) (second (<! (auction-offering-factory/ens)))))
      (is (= (namehash "eth") (second (<! (auction-offering-factory/root-node)))))
      (is (= (contract-address :offering-registry) (second (<! (auction-offering-factory/offering-registry)))))
      (is (= (contract-address :offering-requests) (second (<! (auction-offering-factory/offering-requests)))))

      (is (= (contract-address :ens) (second (<! (buy-now-offering-factory/ens)))))
      (is (= (namehash "eth") (second (<! (buy-now-offering-factory/root-node)))))
      (is (= (contract-address :offering-registry) (second (<! (buy-now-offering-factory/offering-registry)))))
      (is (= (contract-address :offering-requests) (second (<! (buy-now-offering-factory/offering-requests)))))

      (is (= (state/active-address) (last (<! (offering/emergency-multisig (contract-address :buy-now-offering))))))
      (is (= (contract-address :ens) (last (<! (offering/ens (contract-address :buy-now-offering))))))
      (is (= (contract-address :offering-registry) (last (<! (offering/offering-registry (contract-address :buy-now-offering))))))

      (is (= (state/active-address) (last (<! (offering/emergency-multisig (contract-address :auction-offering))))))
      (is (= (contract-address :ens) (last (<! (offering/ens (contract-address :auction-offering))))))
      (is (= (contract-address :offering-registry) (last (<! (offering/offering-registry (contract-address :auction-offering))))))
        ...
        (when offering
          (testing "Offering parameters are correct"
            (is (= (offering-status-keys
                   ...
                   (offering-status-keys (last (<! (offering/get-offering offering)))))))
          ...
          (testing "Making sure an offering isn't too generous too"
            (is (tx-failed?
                  (<! (buy-now-offering/buy! {:offering/address offering} {:value (eth->wei 0.09999)
                                                                           :from (state/my-address 1)})))))
          ...
          (testing "The name ownership must be transferred to the new owner"
            (is (= (state/my-address 1) (last (<! (ens/owner {:ens.record/node (namehash "abc.eth")}))))))
          (testing "Ensuring the new owner gets his deed"
            (is (= (state/my-address 1) (last (<! (registrar/entry-deed-owner {:ens.record/label "abc"}))))))
          (testing "New-owner of the offering is set"
            (is (= (state/my-address 1) (:offering/new-owner (last (<! (offering/get-offering offering)))))))))
      (done))))
              ...
              (fn nearfuture [_]
                (go
                  (testing "User who was overbid, can successfully withdraw funds from auction offering."
                    (is (tx-sent? (<! (auction-offering/withdraw! {:address (state/my-address 1)
                                                                   :offering offering}
                                                                  {:from (state/my-address 1)})))))
                  ...
                  (testing "Ensuring the new owner gets his name"
                    (is (= (state/my-address 3) (last (<! (ens/owner {:ens.record/node (namehash "abc.eth")}))))))
                  (testing "Ensuring the new owner gets his deed"
                    (is (= (state/my-address 3) (last (<! (registrar/entry-deed-owner {:ens.record/label "abc"}))))))
  ...
  (async done
    (go
      (testing "Registering name to transfer onwnership"
        (is (tx-sent? (<! (registrar/register! {:ens.record/label "notowned"}
                                               {:from (state/my-address 0)}))))

        (let [deed-addr
              (:registrar.entry.deed/address
                (second
                  (<! (registrar/entry {:ens.record/label "notowned"}
                                       {:from (state/my-address 0)}))))]
          (is (not (empty? deed-addr)))
          (is (not (empty? (second (<! (deed/owner deed-addr (state/my-address 1)))))))))

      (testing "Registering name to transfer deed"
        (is (tx-sent? (<! (registrar/register! {:ens.record/label "notowndeed"}
                                               {:from (state/my-address 0)}))))
        (is (tx-sent? (<! (registrar/transfer! {:ens.record/label "notowndeed"
                                                :ens.record/owner (state/my-address 0)}
                                               {:from (state/my-address 0)})))))
      (testing
        "Can't make an instant offer on not owned domain"
        (is (tx-failed? (<! (buy-now-offering-factory/create-offering! {:offering/name "notowned"
                                                                        :offering/price (eth->wei 0.1)}
                                                                       {:from (state/my-address 1)})))))
      (testing
        "Can't make an instant offer on not owned domain"
        (is (tx-failed? (<! (buy-now-offering-factory/create-offering! {:offering/name "notowndeed"
                                                                        :offering/price (eth->wei 0.1)}
                                                                       {:from (state/my-address 1)})))))
  ...
  (async done
    (go
      (testing "Registering name to add subdomain"
        ...
        (is (= (state/my-address 0) (last (<! (ens/owner {:ens.record/node (namehash "mysub.tld.eth")})))))
        (is (tx-sent? (<! (ens/set-subnode-owner!
                            {:ens.record/label "theirsub"
                             :ens.record/node "tld.eth"
                             :ens.record/owner (state/my-address 1)}
                            {:from (state/my-address 0)}))))
        (is (= (state/my-address 0) (last (<! (ens/owner {:ens.record/node (namehash "tld.eth")})))))
        (is (= (state/my-address 1) (last (<! (ens/owner {:ens.record/node (namehash "theirsub.tld.eth")}))))))

      (testing
        "Can't make an instant offer if only deed-owner"
        (is (tx-failed? (<! (buy-now-offering-factory/create-offering! {:offering/name "theirsub.tld.eth"
                                                                        :offering/price (eth->wei 0.1)}
                                                                       {:from (state/my-address 0)})))))
      (testing
        "Making an instant offer as an administrator"
        (is (tx-sent? (<! (buy-now-offering-factory/create-offering! {:offering/name "theirsub.tld.eth"
                                                                      :offering/price (eth->wei 0.1)}
                                                                     {:from (state/my-address 1)})))))
      (let [[[_ {{:keys [:offering]} :args}]]
        ...
        (testing
          ...
          (is (tx-failed?
                (<! (buy-now-offering/buy! {:offering/address offering} {:value (eth->wei 0.1) :from (state/my-address 2)})))))
        (testing
          "Transferrnig ownership to the offering"
          (is
            (tx-sent?
              (<! (ens/set-owner! {:ens.record/node (namehash "theirsub.tld.eth")
        ...
        (testing
          ...
          (is (= (state/my-address 2)
                 (last (<! (ens/owner {:ens.record/node (namehash "theirsub.tld.eth")})))))))

      (testing
        "Making an instant offer if not an owner fails"
        (is (tx-failed? (<! (buy-now-offering-factory/create-offering! {:offering/name "mysub.tld.eth"
                                                                        :offering/price (eth->wei 0.1)}
                                                                       {:from (state/my-address 1)})))))

      (testing
        "Making an instant offer as a owner"
        (is (tx-sent? (<! (buy-now-offering-factory/create-offering! {:offering/name "mysub.tld.eth"
                                                                      :offering/price (eth->wei 0.1)}
                                                                     {:from (state/my-address 0)})))))
      (done))))
