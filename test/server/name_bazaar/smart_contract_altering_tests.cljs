      ...
      (let [[[_ {{:keys [:offering]} :args}]]
        ...
        (when offering
          ...
          (testing "The name ownership must be transferred to the offering"
            (is (= offering (last (<! (ens/owner {:ens.record/node (namehash
                                                                        "abc.eth")}))))))

          (testing "Ensuring offering gets the deed"
            (is (= offering (last (<! (registrar/entry-deed-owner {:ens.record/label "abc"}))))))
          (testing "For Buy Now offering, original owner can reclaim ENS name ownership (for TLD also deed ownership)"
            (is (tx-sent? (<! (buy-now-offering/reclaim-ownership! offering {:from (state/my-address 1)})))))
  ...
  (async done
    (go
      ...
      (testing "The name ownership must be transferred to the user"
        (is (= (state/my-address 2) (last (<! (ens/owner {:ens.record/node (namehash
                                                                                "theirsub.tld.eth")}))))))
      (testing "Making an instant offer"
      ...
      (let [[[_ {{:keys [:offering]} :args}]]
        ...
        (when offering
          ...
          (testing "The name ownership must be transferred to the offering"
            (is (= offering (last (<! (ens/owner {:ens.record/node (namehash "theirsub.tld.eth")}))))))

          (testing "For Buy Now offering, original owner can reclaim ENS name ownership"
            (is (tx-sent? (<! (buy-now-offering/reclaim-ownership! offering
                                                                   {:from (state/my-address 2)})))))

          (testing "The name ownership must be transferred back to owner"
            (is (= (state/my-address 2) (last (<! (ens/owner {:ens.record/node (namehash "theirsub.tld.eth")}))))))))
      (done))))
    ...
    (go
      ...
      (let [[[_ {{:keys [:offering]} :args}]]
        ...
        (when offering
          ...
          (testing "The name ownership must be transferred to the offering"
            (is (= offering (last (<! (ens/owner {:ens.record/node (namehash "abc.eth")}))))))
    ...
    (go
      ...
      (let [[[_ {{:keys [:offering]} :args}]]
        ...
        (when offering
          (testing "Transferrnig ownership to the offering"
            (is
              (tx-sent? (<! (ens/set-subnode-owner! {:ens.record/label "theirsub"
                                                     :ens.record/node "tld.eth"
                                                     :ens.record/owner offering}
                                                    {:from (state/my-address 1)})))))
          ...
          (testing "For Buy Now offering, original owner can reclaim ENS name ownership"
            (is (tx-sent? (<! (buy-now-offering/reclaim-ownership! offering {:from (state/my-address 2)})))))
      ...
      (let [[[_ {{:keys [:offering]} :args}]]
        ...
        (when offering
          ...
          (testing "Offering can be successfully edited by original owner, throws error if different address tries to edit"
            (is (tx-failed? (<! (buy-now-offering/set-settings! {:offering/address offering
                                                                 :offering/price (eth->wei 0.2)}
                                                                {:from (state/my-address 2)})))))
          (testing "Updating price"
            (is (tx-sent? (<! (buy-now-offering/set-settings! {:offering/address offering
                                                               :offering/price (eth->wei 0.2)}
                                                              {:from (state/my-address 1)})))))
          (testing "The price is updated"
            (is (= (eth->wei 0.2) (str (:offering/price (last (<! (offering/get-offering offering))))))))
  ...
  (async done
    (go
      ...
      (testing "user can successfully request ENS name"
        (is (tx-sent? (<! (offering-requests/add-request! {:offering-request/name "abc.eth"}
                                                          {:form (state/my-address 1)})))))
      (testing "and it increase counter of requests."
        (is (= 1 (:offering-request/requesters-count
                   (second (<! (offering-requests/get-request {:offering-request/node (namehash "abc.eth")})))))))
      (testing "Making an instant offer"
        (is (tx-sent? (<! (buy-now-offering-factory/create-offering! {:offering/name "abc.eth"
                                                                      :offering/price (eth->wei 0.1)}
                                                                     {:from (state/my-address 1)})))))
