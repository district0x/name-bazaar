        ...
        (if (= 1 (count @my-addresses))
          (when-not @xs-width?
            [:div
             ...
             (d0x-ui-utils/truncate @active-address (or (:single-address-truncate single-address-props) 13))])
          [ui/select-field
           (r/merge-props
           ...
           (for [address @my-addresses]
             [ui/menu-item
              (r/merge-props
                {:value address
                 :primary-text (d0x-ui-utils/truncate address (or (:menu-item-truncate menu-item-props) 25))
                 :key address}
                menu-item-props)])])))))
